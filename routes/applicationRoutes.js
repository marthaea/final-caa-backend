...
router.get('/export', verifyToken, requirePerm('canViewApplications'), async (req, res) => {
  try {
    const { jobId, status, fromDate, toDate, email } = req.query;
    const conditions = [];
    const params = [];
    ...
    console.error('GET /applications/export:', e);
    return res.status(500).json({ success: false, error: 'Internal server error' });
  }
});

// POST /api/applications
router.post('/', verifyToken, submitApplicationRules, validate, async (req, res) => {
  try {
...
router.put('/bulk-status', verifyToken, requirePerm('canShortlist'), bulkStatusRules, validate, async (req, res) => {
  try {
    ...
    console.error('PUT /applications/bulk-status:', e);
    return res.status(500).json({ success: false, error: 'Internal server error' });
  }
});

// PUT /api/applications/:id/status
router.put('/:id/status', verifyToken, requirePerm('canShortlist'), statusUpdateRules, validate, async (req, res) => {
  try {
    ...
    console.error('PUT /applications/:id/status:', e);
    return res.status(500).json({ success: false, error: 'Internal server error' });
  }
});
